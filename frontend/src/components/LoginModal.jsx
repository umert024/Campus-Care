...
export const LoginModal = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fadeIn">
      <div className="w-full max-w-md overflow-hidden rounded-3xl border border-violet-200 bg-white shadow-2xl shadow-violet-950/20">
        <div className="relative overflow-hidden bg-gradient-to-br from-violet-700 via-indigo-700 to-sky-700 p-7 text-white">
          <div className="absolute -right-8 -top-12 h-40 w-40 rounded-full bg-cyan-300/20" />
          <div className="relative flex items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-violet-100">Your facility workspace, your own account.</p>
            </div>
            {currentUser && <button onClick={() => setIsLoginModalOpen(false)} className="rounded-xl p-2 text-violet-100 hover:bg-white/10 hover:text-white"><IconClose className="h-5 w-5" /></button>}
          </div>
        </div>

        <div className="p-6">
          <div className="mb-6 grid grid-cols-2 rounded-xl bg-slate-100 p-1 text-sm font-bold">
            <button onClick={() => { setMode('login'); setMessage(''); }} className={`rounded-lg py-2 transition ${mode === 'login' ? 'bg-white text-violet-700 shadow-sm' : 'text-slate-500'}`}>Sign in</button>
            <button onClick={() => { setMode('signup'); setMessage(''); }} className={`rounded-lg py-2 transition ${mode === 'signup' ? 'bg-white text-violet-700 shadow-sm' : 'text-slate-500'}`}>Create account</button>
          </div>
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            {message && <p role="alert" className="rounded-xl bg-rose-50 px-3 py-2 text-xs font-medium text-rose-700">{message}</p>}
            <button disabled={busy} className="w-full rounded-xl bg-gradient-to-r from-violet-700 to-indigo-700 py-3 text-sm font-bold text-white shadow-lg shadow-violet-700/20 transition hover:from-violet-800 hover:to-indigo-800 disabled:opacity-60">{busy ? 'Please wait…' : mode === 'login' ? 'Sign in to workspace' : 'Create my account'}</button>
          </form>
          {mode === 'login' && <p className="mt-4 text-center text-xs text-slate-500">Demo accounts use their shown usernames and password <span className="font-bold text-slate-700">campus123</span>.</p>}
