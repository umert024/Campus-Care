...
export const FacilityProvider = ({ children }) => {
  ...
  const [isLoginModalOpen, setIsLoginModalOpen] = useState(() => !localStorage.getItem('cf_current_user'));
  ...
  const finishLogin = (userObj) => {
    ...
    if (userObj.role === 'Technician') setActiveTab('tech-queue');
    else if (userObj.role === 'Admin') setActiveTab('dashboard');
    else if (userObj.role === 'Faculty') setActiveTab('inventory');
  ...
  const login = async ({ username, password }) => {
    if (isBackendConnected) {
      try {
        ...
        setUsers(previous => previous.some(u => u.id === data.user.id) ? previous : [...previous, data.user]);
        finishLogin(data.user);
        return { success: true };
      } catch (error) { /* local mode below */ }
    }
  ...
  const signup = async ({ name, username, password, role, department }) => {
    ...
    if (!name.trim() || !cleanUsername || !password) return { success: false, message: 'Name, username, and password are required.' };
    if (!/^[a-z0-9._-]{3,24}$/.test(cleanUsername)) return { success: false, message: 'Use 3–24 letters, numbers, dots, dashes, or underscores for the username.' };
    ...
    if (isBackendConnected) {
      try {
        const response = await fetch(`${API_URL}/auth/signup`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name, username: cleanUsername, password, role, department })
        });
        const data = await response.json();
    ...
    if (users.some(u => u.username?.toLowerCase() === cleanUsername)) return { success: false, message: 'That username is already in use.' };
    const avatarByRole = { Admin: '👨‍💼', Technician: '🛠️', Faculty: '👨‍🏫', Student: '🎓' };
    const user = { id: `USR-${Date.now()}`, name: name.trim(), username: cleanUsername, password, role, department: department.trim() || 'Campus Community', avatar: avatarByRole[role] || '🎓', title: role === 'Student' ? 'Campus Member' : `${role} Member` };
  ...
  const addIssue = async (issueData) => {
    ...
    const newIssue = {
      ...
      updates: [
        { timestamp: nowStr, text: `Issue reported by ${reporterName}.` }
      ]
    };
    ...
    setIssues(prev => prev.map(issue => {
      if (issue.id === issueId) {
        ...
        if (issue.equipmentId) {
          ...
          if (newStatus === 'Under repair' || newStatus === 'In Progress') eqStatus = 'Under Repair';
          else if (newStatus === 'Reported') eqStatus = 'Needs Service';
  ...
  const underRepairCount = issues.filter(i => i.status?.toLowerCase() === 'under repair').length;
  const reportedCount = issues.filter(i => i.status?.toLowerCase() === 'reported').length;
  const resolvedCount = issues.filter(i => i.status?.toLowerCase() === 'resolved').length;

  // Filter issues assigned to current technician if logged in as Technician
  const techWorkOrders = currentUser?.role === 'Technician'
    ? issues.filter(i => i.assignedTechnician?.toLowerCase().includes(currentUser.name.toLowerCase()) || i.assignedTechnician === 'Unassigned')
    : [];

  const totalEquipmentCount = equipment.length;
  const operationalEqCount = equipment.filter(e => e.status === 'Operational').length;
